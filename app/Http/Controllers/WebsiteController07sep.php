...
class WebsiteController extends Controller
{
                ...
                foreach ($generatedPlans as $plan) {
                    ...
                    if ($meal) {
                        if (!empty($allergy_ids)) {
                            $mealAllergies = explode(',', $meal->allergy_id);
    ...
    public function getMealData($mealId)
    {
        ...
        $meal = DB::table('food_menu')
                  ->join('allergy', 'food_menu.allergy_id', '=', 'allergy.allergy_id')
                  ->select('food_menu.*', 'allergy.allergy_name')
        ...
        if ($meal) {
            return response()->json([
                'status' => 'success',
                'meal' => $meal,
                'allergy_name' => $meal->allergy_name, // Return the allergy name
                'allergy_ids' => $meal->allergy_id
            ]);
        } else {
            return response()->json([
                'status' => 'error',
    ...
    public function user_account()
    {
        ...
        if(!empty($data))
        {
            ...
            return view('web.myaccount', compact('data', 'orders'));
        }
        else
        {
    ...
    //         ->join('user_menus', 'user_order.order_id', '=', 'user_menus.order_id')
    //         ->join('food_menu', 'user_menus.menu_id', '=', 'food_menu.menu_id')
    //         ->join('meal_plan', 'user_order.meal_id', '=', 'meal_plan.meal_id')
    //         ->join('delivery_status', 'user_order.delivery_id', '=', 'delivery_status.delivery_id')
    //         ->join('generated_plans', 'generated_plans.menu_id', '=', 'food_menu.menu_id')
    ...
    //             'food_menu.*',
    //             'generated_plans.day',
    //             'generated_plans.week',
    //             'generated_plans.meal_type'
    //         )
    //         ->orderBy('generated_plans.day')
    //         ->get()
    //         ->groupBy('day');
